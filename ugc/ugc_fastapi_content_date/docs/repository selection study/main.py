...
for users in users_list:
    ...
    for movies in movies_list:
        ...
        while favorites <= max_strings:
            favorites += 1
            favorites_doc = {"user_id": user_id, "movies_id": movies_id}
...
print("Время записи")
print("Время чтения")
pipeline = [{"$group": {"_id": {"field1": "$user_id", "field2": "$movies_id"}}}]
...
pipeline = [
    ...
    {
        "$group": {
            "_id": {"field1": "$user_id", "field2": "$movies_id"},
            "avgValue": {"$avg": "$likes"},
        }
    },
]
...
end_time = datetime.now()
print(
    f"список понравившихся пользователю фильмов (список лайков пользователя) = {end_time - start_time} "
)

pipeline = [
    {"$match": {"movies_id": movies_list[1]}},
    {"$group": {"_id": {"field1": "$movies_id"}, "avgValue": {"$avg": "$likes"}}},
]
start_time = datetime.now()
result = likes_collection.aggregate(pipeline)
end_time = datetime.now()
...
pipeline = [
    {"$match": {"user_id": users_list[1]}},
    {"$group": {"_id": {"field1": "$user_id"}, "count": {"$sum": 1}}},
]
start_time = datetime.now()
