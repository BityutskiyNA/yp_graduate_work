...
def test_mailing_type_view(client, mailing_type, mail_account, mailing_list, mailing_data, html_content, system_events):
    expected_response = {
    ...
    response = client.get(f"/mailing/system_events/{system_events.events_name}/")
...
@pytest.mark.django_db
def test_mailing_type_view_invalid_query(client, system_events):

    response = client.post(f"/mailing/system_events/{system_events.events_name}/")
